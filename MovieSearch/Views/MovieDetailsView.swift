...
class MovieDetailsView: UIView {
    ...
    private var _posterImv = UIImageView()
    ...
    func configure(with movie: MovieSearchResult) {
        ...
        _posterImv.downloadImage(from: movie.fullPosterPath, placeholder: Constants.placeholderImage)
    }
    
    func configure(with credits: MovieCreditsResult) {
        _castLabel.text = "Cast: " + (credits.castString ?? "...")
        _crewLabel.text = "Crew: " + (credits.crewString ?? "...")
    }
}
